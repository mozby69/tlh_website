 ...
 *
 * Plays the TLH splash for installed mobile PWA launches/resumes and for
 * desktop/tablet website entries. Internal navigations use a short-lived marker
 * so the animation does not replay between normal screens. Browser tabs do not
 * replay the splash merely because the user switches back to them.
 */
(function () {
  'use strict';
  ...
  const launchContext = (appContext && mobileViewport) || tabletOrDesktopViewport;

  // Mark app mode before stylesheets finish loading so phone-sized PWA chrome
  // can differ from browser/tablet navigation without a visible flash.
  if (appContext) document.documentElement.classList.add('tlh-standalone-app');

  // Keep ordinary phone-browser visits splash-free. Desktop/tablet website
  // entries use the branded splash, while installed mobile PWAs retain their
